...
export async function getUserLimits(user: User): Promise<any> {
...
export async function incrementUrlCount(user: User) {
  const key = `limits:${user.uid}`;

  return kv.hincrby(key, "usedToday", 1);
}

export async function getRemainingUrls(user: User) {
